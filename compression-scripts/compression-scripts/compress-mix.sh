...
# Pass 2: The actual encode with Mixed Audio
ffmpeg -y -i "$INPUT" \
-filter_complex "[0:a:0][0:a:1]amix=inputs=2:duration=first[aout]" \
-map 0:v -map "[aout]" \
-c:v libx265 -b:v "${VIDEO_BITRATE}k" -x265-params pass=2 -vf "scale=1280:-2,fps=30" \
-c:a aac -b:a "${AUDIO_BITRATE}k" -tag:v hvc1 "$OUTPUT"
